...
public:
	...
	void SetPhysicsFramerate (float ffr) { physics_framerate = ffr; }
	//Data from other classes to display
	void increaseScore() { score++; }
	void decreaseScore() { if (score > 0) { score = 0; } else if (score <= 0) { score -= 10; } }
	void setMeatballs(int val) {meatballs = val;}
	void setGreanades (int val) { greanades = val; }
	void setCurrent(int val) { current = val; }
	void setBallSize(float val) { ballsize = val; }
...
protected:	
	...
	void	DrawSkyBox();
	void DrawHeightMap (Mesh m);
	void DrawMessage();
	void PutText(const std::string &text, const Vector3 &position, const float size = 10.0f, const bool perspective = false);
	

	SceneNode*	root;
	Camera*		camera;
	Shader*		simpleShader;
	//The shader used to load the skybox
	Shader*		skyBoxShader;
	Shader*		textShader;
