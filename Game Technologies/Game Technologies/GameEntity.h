...
public:
	...
	void			SetRenderNode(SceneNode* sn) { renderNode = sn; }
	PhysicsNode&	GetPhysicsNode()	{ return *physicsNode; }
	void			SetPhysicsNode(PhysicsNode* pn) { physicsNode = pn; }

	void			ConnectToSystems();
	void			DisconnectFromSystems();
	void setMeat(bool val) { isMeat = val; }
	bool getMeat() { return isMeat; }
	//This method eliminates an element, used for eliminating off view elements 
	void dissappear();
